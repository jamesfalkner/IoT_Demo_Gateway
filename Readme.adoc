...
1. LOCAL_BROKER_URL is the IP-address and port for the broker running inside the Smart Gateway. Usually there is no reason to change is entry.

2. DATACENTER_BROKER_URL is the IP-address and port for the broker in the https://github.com/PatrickSteiner/IoT_Demo_Datacenter[Datacenter] part of the demo. This should reflect the IP-Adress of the docker host for the Datacenter part.
